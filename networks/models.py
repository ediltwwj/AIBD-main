...
class Generator(nn.Sequential):
    ...
    def _get_denormalize(self, opt):
        if opt.dataset == "cifar10" or opt.dataset == "CIFAR10":
            # denormalizer = Denormalize(opt, [0.4914, 0.4822, 0.4465], [0.247, 0.243, 0.261])
            denormalizer = Denormalize(opt, (0.4914, 0.4822, 0.4465), (0.2023, 0.1994,0.2010))
        elif opt.dataset == "cifar100" or opt.dataset == "CIFAR100":
            denormalizer = Denormalize(opt, (0.5071, 0.4867, 0.4408), (0.2675, 0.2565, 0.2761))
        elif opt.dataset == "mnist" or opt.dataset == "MNIST":
            denormalizer = Denormalize(opt, [0.5], [0.5])
        elif opt.dataset == "gtsrb" or opt.dataset == "GTSRB":
            denormalizer = None
        elif opt.dataset == "imgnet-subset-30" or opt.dataset == "ImageNet-Subset-30":
    ...
    def _get_normalize(self, opt):
        if opt.dataset == "cifar10"or opt.dataset == "CIFAR10":
            ...
            normalizer = Normalize(opt, (0.5071, 0.4867, 0.4408), (0.2675, 0.2565, 0.2761))
        elif opt.dataset == "mnist" or opt.dataset == "MINST":
